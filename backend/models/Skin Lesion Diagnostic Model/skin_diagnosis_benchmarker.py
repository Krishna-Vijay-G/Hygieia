...
# -*- coding: utf-8 -*-
"""
Benchmark Skin Diagnosis Model Accuracy

python models/Skin Lesion Diagnostic Model/skin_diagnosis_benchmarker.py --multi-seed 123 456 789 --samples 7

Model v4.0 Performance: 95.9% peak accuracy, 93.9% mean production accuracy on 8,039 samples
This script evaluates the accuracy of the Skin Diagnosis Model on a subset of the HAM10000 dataset.
...
# Add parent directories to path to import from backendnew folder
current_dir = os.path.dirname(os.path.abspath(__file__))
backend_dir = os.path.dirname(os.path.dirname(current_dir))  # Go up two levels to backendnew
...
# Controller path
CONTROLLERS_PATH = os.path.join(backend_dir, 'controllers')
...
class SkinDiagnosisBenchmarker:
    """
    Skin Diagnosis Model Benchmarking Class
    Evaluates skin diagnosis model performance using SkinDiagnosisIntegration
    """

    def __init__(self, config=None):
        """Initialize the benchmarker"""
        self.config = config or BENCHMARK_CONFIG.copy()
        self.skin_diagnosis_model_class = None
        self.predictor = None

        # Load the skin diagnosis model class
        self._load_model_class()

    def _load_model_class(self):
        """Load the skin diagnosis model integration class"""
        try:
            module_file = os.path.join(CONTROLLERS_PATH, 'skin_diagnosis.py')
            spec = importlib.util.spec_from_file_location('skin_diagnosis', module_file)
            module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(module)
            self.skin_diagnosis_model_class = getattr(module, 'SkinDiagnosisIntegration', None)
            if self.skin_diagnosis_model_class:
                logger.info("Successfully loaded SkinDiagnosisIntegration class")
            else:
                logger.error("SkinDiagnosisIntegration class not found")
        except Exception as e:
            logger.error(f"Failed to load skin diagnosis model class: {e}")
...
def evaluate_model(test_df):
    ...
    benchmarker = SkinDiagnosisBenchmarker()
    if not benchmarker.skin_diagnosis_model_class:
        raise RuntimeError("Skin diagnosis model class not loaded")
    
    predictor = benchmarker.skin_diagnosis_model_class()
    ...
    for idx, row in test_df.iterrows():
        ...
        try:
            # Get prediction from model using controller directly
...
def calculate_metrics(results, true_labels, pred_labels):
    ...
    # ROC AUC calculation (multi-class using one-vs-rest)
    try:
        class_list = list(CONDITION_MAPPING.keys())
        # Build score matrix
        y_score = np.zeros((len(results), len(class_list)))
        y_true_arr = np.array(true_labels)
        for i, res in enumerate(results):
            probs = res.get('probabilities', {})
            for j, c in enumerate(class_list):
                y_score[i, j] = probs.get(c, 0.0)

        # Binarize true labels
        y_true_bin = label_binarize(y_true_arr, classes=class_list)

        # Compute macro and weighted ROC AUC
        if y_score.shape[0] > 0 and y_score.shape[1] == y_true_bin.shape[1]:
            roc_macro = roc_auc_score(y_true_bin, y_score, multi_class='ovr', average='macro')
            roc_weighted = roc_auc_score(y_true_bin, y_score, multi_class='ovr', average='weighted')
            print_color(f"\nAUC-ROC (macro): {roc_macro:.3f}", Fore.GREEN)
            print_color(f"AUC-ROC (weighted): {roc_weighted:.3f}", Fore.GREEN)

            # Per-class AUC
            for j, c in enumerate(class_list):
                # Only compute if class has positive examples
                if np.sum(y_true_bin[:, j]) > 0:
                    auc_j = roc_auc_score(y_true_bin[:, j], y_score[:, j])
                    print_color(f"  {c}: AUC = {auc_j:.3f}", Fore.CYAN)
        else:
            print_color("AUC-ROC calculation skipped: incompatible shapes", Fore.YELLOW)
    except Exception as e:
        print_color(f"AUC-ROC calculation error: {e}", Fore.RED)
...
def main():
    """Main function"""
    parser = argparse.ArgumentParser(description='Benchmark the skin diagnosis model accuracy')
    ...
    print_section_header("SKIN DIAGNOSIS MODEL BENCHMARK")
